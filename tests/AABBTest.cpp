...
TEST_F(AABBTest, Construction)
{
    ...
    EXPECT_EQ(box.halfSize, Vec2(1.5f, 2.0f));  // halfSize should be half of input size
}

TEST_F(AABBTest, ContainsPoint)
{
    AABB box(Vec2(0, 0), Vec2(4, 4));  // Full size 4x4, halfSize will be 2x2
...
{
    AABB box1(Vec2(0, 0), Vec2(4, 4));  // Full size 4x4, halfSize 2x2

    // Test overlapping boxes
    EXPECT_TRUE(box1.intersects(AABB(Vec2(1, 1), Vec2(4, 4))));  // Partial overlap
    EXPECT_TRUE(box1.intersects(AABB(Vec2(0, 0), Vec2(2, 2))));  // Contained
    EXPECT_TRUE(box1.intersects(AABB(Vec2(0, 0), Vec2(6, 6))));  // Contains
    EXPECT_TRUE(box1.intersects(box1));                          // Self intersection

    // Test touching boxes (should intersect)
    EXPECT_TRUE(box1.intersects(AABB(Vec2(4, 0), Vec2(4, 4))));  // Edge touch
    EXPECT_TRUE(box1.intersects(AABB(Vec2(4, 4), Vec2(4, 4))));  // Corner touch

    // Test non-overlapping boxes
    EXPECT_FALSE(box1.intersects(AABB(Vec2(5, 0), Vec2(2, 2))));  // Separated horizontally
    EXPECT_FALSE(box1.intersects(AABB(Vec2(0, 5), Vec2(2, 2))));  // Separated vertically
    EXPECT_FALSE(box1.intersects(AABB(Vec2(5, 5), Vec2(2, 2))));  // Separated diagonally
}

TEST_F(AABBTest, SymmetricIntersection)
{
    // Test that intersection is symmetric (A intersects B iff B intersects A)
    AABB box1(Vec2(0, 0), Vec2(4, 4));
    AABB box2(Vec2(1, 1), Vec2(4, 4));
    AABB box3(Vec2(5, 5), Vec2(2, 2));
...
TEST_F(AABBTest, EdgeCases)
{
    ...
    AABB normalBox(Vec2(0, 0), Vec2(2, 2));  // Full size 2x2, halfSize 1x1
    ...
    AABB negativeBox(Vec2(0, 0), Vec2(-4, -4));  // Full size 4x4, halfSize 2x2 (abs value)
