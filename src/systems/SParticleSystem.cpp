...
void SParticleSystem::render(sf::RenderWindow* window)
{
    ...
    for (auto& entity : entities)
    {
        if (entity->hasComponent<CParticleEmitter>() == false)
        {
            ...
            if (std::find(s_entitiesWithoutEmitters.begin(), s_entitiesWithoutEmitters.end(), entity->getId()) == s_entitiesWithoutEmitters.end())
            {
        ...
        for (const auto& particle : emitter->particles)
        {
            ...
            sf::Color color(particle.color.r, particle.color.g, particle.color.b,
                           static_cast<sf::Uint8>(particle.alpha * 255.0f));
        ...
        if (m_vertexArray.getVertexCount() > 0)
        {
            ...
            if (emitter->particles.size() > 0 && emitter->particles[0].alive)
            {
                ...
                std::cout << "First particle - World: (" << p.position.x << ", " << p.position.y 
                          << ") Screen: (" << screenPos.x << ", " << screenPos.y 
                          << ") Size: " << pixelSize << "px" << std::endl;
            }
            
            std::cout << "Rendering " << (m_vertexArray.getVertexCount() / 4) << " particles for entity " << entity->getId() 
                      << " (texture: " << (emitter->texture ? "YES" : "NO") 
                      << ", texSize: " << (emitter->texture ? std::to_string(emitter->texture->getSize().x) + "x" + std::to_string(emitter->texture->getSize().y) : "N/A")
                      << ")" << std::endl;
