            ...
            if (shapeTypeValue.isString())
            {
                ...
                else if (typeStr == "Polygon")
                {
                    ...
                    if (verticesValue.isArray())
                    {
                        ...
                        int count = std::min(static_cast<int>(vertices.size()), static_cast<int>(B2_MAX_POLYGON_VERTICES));
                        for (int i = 0; i < count; ++i)
                        {
                            if (vertices[i].isObject())
                            {
                                ...
                                {
                                    fixture.shapeData.polygon.vertices[i].x = static_cast<float>(xValue.getNumber());
                                    fixture.shapeData.polygon.vertices[i].y = static_cast<float>(yValue.getNumber());
                                }
                            }
                        }
                    }
